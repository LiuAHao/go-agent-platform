...
func (s *Store) EnsureSeedData(cfg config.Config) error {
	...
	_, err = s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, "admin-001", cfg.SeedAdminEmail, "Admin", shared.HashPassword(cfg.SeedAdminPassword), "super_admin", "active", now, now)
	if err != nil {
	...
	_, err = s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?)
	`, "default", "Default Workspace", "Default workspace", "admin-001", now)
	if err != nil {
	...
	_, err = s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?)
	`, "member-001", "default", "admin-001", "owner", now)
	if err != nil {
...
func (s *Store) SaveSessionToken(token auth.SessionToken) error {
	_, err := s.db.Exec(`
		...
		VALUES (?, ?, ?, ?)
	`, token.Token, token.UserID, token.ExpiresAt, time.Now().UTC())
	return err
}
...
func (s *Store) CreateWorkspace(item workspace.Workspace, member workspace.Membership) error {
	...
	_, err = tx.Exec(`
		INSERT INTO workspace_members (id, workspace_id, user_id, role, created_at)
		VALUES (?, ?, ?, ?, ?)
	`, shared.NewID("member"), member.WorkspaceID, member.UserID, string(member.Role), member.CreatedAt)
	if err != nil {
		return err
	}
...
func (s *Store) FindAgentVersion(versionID string) (agent.Version, error) {
	...
	`, versionID).Scan(&v.ID, &v.AgentID, &v.VersionNumber, &snapshot, &v.CreatedBy, &v.CreatedAt)
	if err != nil {
...
func (s *Store) SaveTool(item tool.Tool) error {
	_, err := s.db.Exec(`
		INSERT INTO tools (id, workspace_id, name, scope, description, schema_json, config_json, requires_approval, enabled, created_by, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, item.ID, item.WorkspaceID, item.Name, item.Scope, item.Description,
		toJSON(item.Schema), toJSON(item.Config), item.RequiresApproval, item.Enabled,
...
func (s *Store) UpdateTool(item tool.Tool) error {
	_, err := s.db.Exec(`
		UPDATE tools
		SET name = ?, description = ?, schema_json = ?, config_json = ?, requires_approval = ?, enabled = ?
		WHERE id = ?
	`, item.Name, item.Description,
		toJSON(item.Schema), toJSON(item.Config), item.RequiresApproval, item.Enabled, item.ID)
	return err
}
...
func (s *Store) listToolsByQuery(query string, args ...any) ([]tool.Tool, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.WorkspaceID, &t.Name, &t.Scope, &t.Description,
			&schemaData, &configData, &requiresApproval, &enabled,
			&t.CreatedBy, &t.CreatedAt); err != nil {
			continue
		}
		t.RequiresApproval = requiresApproval == 1
		t.Enabled = enabled == 1
...
func (s *Store) FindToolByID(toolID string) (tool.Tool, error) {
	...
	var requiresApproval, enabled int
	err := s.db.QueryRow(`
		SELECT id, workspace_id, name, scope, description, schema_json, config_json, requires_approval, enabled, created_by, created_at
		FROM tools WHERE id = ?
...
func (s *Store) InstallTool(userID, toolID string) error {
	_, err := s.db.Exec(`
		INSERT INTO install_records (id, user_id, resource_type, resource_id, installed_at)
		VALUES (?, ?, 'tool', ?, ?)
		ON DUPLICATE KEY UPDATE installed_at = VALUES(installed_at)
	`, shared.NewID("install"), userID, toolID, time.Now().UTC())
	return err
}

func (s *Store) UninstallTool(userID, toolID string) error {
	_, err := s.db.Exec("DELETE FROM install_records WHERE user_id = ? AND resource_type = 'tool' AND resource_id = ?", userID, toolID)
	return err
}

func (s *Store) ListInstalledToolIDs(userID string) ([]string, error) {
	rows, err := s.db.Query("SELECT resource_id FROM install_records WHERE user_id = ? AND resource_type = 'tool'", userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) SaveSkill(item skill.Skill) error {
	_, err := s.db.Exec(`
		INSERT INTO skills (id, workspace_id, name, slug, scope, description, version, entry, schema_json, config_json, enabled, created_by, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func (s *Store) UpdateSkill(item skill.Skill) error {
	_, err := s.db.Exec(`
		UPDATE skills
		SET name = ?, description = ?, version = ?, entry = ?, schema_json = ?, config_json = ?, enabled = ?, updated_at = ?
		WHERE id = ?
...
func (s *Store) FindSkillByID(skillID string) (skill.Skill, error) {
	...
	var enabled int
	err := s.db.QueryRow(`
		SELECT id, workspace_id, name, slug, scope, description, version, entry, schema_json, config_json, enabled, created_by, created_at, updated_at
		FROM skills WHERE id = ?
	`, skillID).Scan(&sk.ID, &sk.WorkspaceID, &sk.Name, &sk.Slug, &sk.Scope, &sk.Description, &sk.Version,
		&sk.Entry, &schemaData, &configData, &enabled,
...
func (s *Store) ListPlatformSkills() ([]skill.Skill, error) {
	return s.listSkillsByQuery("SELECT * FROM skills WHERE scope = 'platform' AND status = 'published'")
}
...
func (s *Store) InstallSkill(userID, skillID string) error {
	_, err := s.db.Exec(`
		...
		VALUES (?, ?, 'skill', ?, ?)
		ON DUPLICATE KEY UPDATE installed_at = VALUES(installed_at)
	`, shared.NewID("install"), userID, skillID, time.Now().UTC())
	return err
}

func (s *Store) UninstallSkill(userID, skillID string) error {
	_, err := s.db.Exec("DELETE FROM install_records WHERE user_id = ? AND resource_type = 'skill' AND resource_id = ?", userID, skillID)
	return err
}

func (s *Store) ListInstalledSkillIDs(userID string) ([]string, error) {
	rows, err := s.db.Query("SELECT resource_id FROM install_records WHERE user_id = ? AND resource_type = 'skill'", userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) SaveModel(item model.Model) error {
	_, err := s.db.Exec(`
		INSERT INTO models (id, workspace_id, name, provider, api_base_url, api_key_encrypted, model_key, description, context_window, max_output_tokens, is_default, enabled, created_by, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, item.ID, item.WorkspaceID, item.Name, item.Provider, item.APIBaseURL,
		item.APIKey, item.ModelKey, item.Description, item.ContextWindow, item.MaxOutputTokens,
		item.IsDefault, item.Enabled, item.CreatedBy, item.CreatedAt, item.UpdatedAt)
	return err
}
...
func (s *Store) UpdateModel(item model.Model) error {
	_, err := s.db.Exec(`
		UPDATE models
		SET name = ?, provider = ?, api_base_url = ?, api_key_encrypted = ?, model_key = ?, description = ?, context_window = ?, max_output_tokens = ?, is_default = ?, enabled = ?, updated_at = ?
		WHERE id = ?
	`, item.Name, item.Provider, item.APIBaseURL, item.APIKey, item.ModelKey,
		item.Description, item.ContextWindow, item.MaxOutputTokens, item.IsDefault, item.Enabled,
...
func (s *Store) FindModelByID(modelID string) (model.Model, error) {
	...
	err := s.db.QueryRow(`
		SELECT id, workspace_id, name, provider, api_base_url, api_key_encrypted, model_key, description, context_window, max_output_tokens, is_default, enabled, created_by, created_at, updated_at
		FROM models WHERE id = ?
	`, modelID).Scan(&m.ID, &m.WorkspaceID, &m.Name, &m.Provider, &m.APIBaseURL,
		&m.APIKey, &m.ModelKey, &m.Description, &m.ContextWindow, &m.MaxOutputTokens,
		&isDefault, &enabled, &m.CreatedBy, &m.CreatedAt, &m.UpdatedAt)
	if err != nil {
...
func (s *Store) ListModels(workspaceID string) ([]model.Model, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.WorkspaceID, &m.Name, &m.Provider, &m.APIBaseURL,
			&m.APIKey, &m.ModelKey, &m.Description, &m.ContextWindow, &m.MaxOutputTokens,
			&isDefault, &enabled, &m.CreatedBy, &m.CreatedAt, &m.UpdatedAt); err != nil {
			return nil, err
		}
		m.IsDefault = isDefault == 1
		m.Enabled = enabled == 1
...
func (s *Store) SaveMessage(item session.Message) error {
	_, err := s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?)
	`, item.ID, item.SessionID, string(item.Role), item.Content, item.CreatedAt)
	return err
}
...
func (s *Store) SaveTask(item task.Task) error {
	_, err := s.db.Exec(`
		INSERT INTO tasks (id, trace_id, workspace_id, agent_id, session_id, model, reasoning, prompt, status, result, error, created_by, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, item.ID, item.TraceID, item.WorkspaceID, item.AgentID, item.SessionID, item.Model, item.Reasoning,
...
func (s *Store) FindTaskByID(taskID string) (task.Task, error) {
	var t task.Task
	err := s.db.QueryRow(`
		SELECT id, trace_id, workspace_id, agent_id, session_id, model, reasoning, prompt, status, result, error, created_by, created_at, updated_at
		FROM tasks WHERE id = ?
	`, taskID).Scan(&t.ID, &t.TraceID, &t.WorkspaceID, &t.AgentID, &t.SessionID, &t.Model, &t.Reasoning,
...
func (s *Store) ListTasksBySession(sessionID string) ([]task.Task, error) {
	...
	for rows.Next() {
		var t task.Task
		if err := rows.Scan(&t.ID, &t.TraceID, &t.WorkspaceID, &t.AgentID, &t.SessionID, &t.Model, &t.Reasoning,
			&t.Prompt, &t.Status, &t.Result, &t.Error,
...
func (s *Store) SaveAuditEvent(item audit.Event) error {
	_, err := s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?)
	`, item.ID, item.ActorID, item.Action, item.Resource, item.ResourceID,
		toJSON(item.Metadata), item.CreatedAt)
	return err
}
...
func (s *Store) ListAuditEvents(workspaceID string) ([]audit.Event, error) {
	...
	for rows.Next() {
		var e audit.Event
		var metadata []byte
		if err := rows.Scan(&e.ID, &e.ActorID, &e.Action, &e.Resource, &e.ResourceID, &metadata, &e.CreatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) GetUserSettings(userID string) (settings.UserSettings, error) {
	...
	err := s.db.QueryRow(`
		SELECT user_id, retention_max_age_days, retention_max_messages, retention_max_size_mb, retention_auto_clean,
		       backup_enabled, backup_frequency, backup_encrypt, backup_max_days, last_backup_at
		FROM user_settings WHERE user_id = ?
	`, userID).Scan(&us.UserID, &us.RetentionMaxAgeDays, &us.RetentionMaxMessages, &us.RetentionMaxSizeMB,
		&us.RetentionAutoClean, &us.BackupEnabled, &us.BackupFrequency, &us.BackupEncrypt,
		&us.BackupMaxDays, &us.LastBackupAt)
	if err != nil {
		// 返回默认设置
...
func (s *Store) SaveUserSettings(us settings.UserSettings) error {
	_, err := s.db.Exec(`
		INSERT INTO user_settings (user_id, retention_max_age_days, retention_max_messages, retention_max_size_mb, retention_auto_clean,
		       backup_enabled, backup_frequency, backup_encrypt, backup_max_days, last_backup_at, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON DUPLICATE KEY UPDATE
			retention_max_age_days = VALUES(retention_max_age_days),
			retention_max_messages = VALUES(retention_max_messages),
			retention_max_size_mb = VALUES(retention_max_size_mb),
			retention_auto_clean = VALUES(retention_auto_clean),
			backup_enabled = VALUES(backup_enabled),
			backup_frequency = VALUES(backup_frequency),
			backup_encrypt = VALUES(backup_encrypt),
			backup_max_days = VALUES(backup_max_days),
			last_backup_at = VALUES(last_backup_at),
			updated_at = VALUES(updated_at)
	`, us.UserID, us.RetentionMaxAgeDays, us.RetentionMaxMessages, us.RetentionMaxSizeMB,
		us.RetentionAutoClean, us.BackupEnabled, us.BackupFrequency, us.BackupEncrypt,
		us.BackupMaxDays, us.LastBackupAt, time.Now().UTC(), time.Now().UTC())
	return err
}
